...
void publish_points(T &new_pc, const sensor_msgs::PointCloud2 &old_msg)
{
    ...
    pc_new_msg.header.frame_id = old_msg.header.frame_id;                         //!复用原始雷达驱动中的frame_id（也可以写死为"velodyne")
...
void hesaiHandler_XYZIRT(const sensor_msgs::PointCloud2 &pc_msg)                  //!核心函数
{
