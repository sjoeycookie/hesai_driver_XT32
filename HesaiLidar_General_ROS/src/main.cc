...
public:
  HesaiLidarClient(ros::NodeHandle node, ros::NodeHandle nh)
  {
    ...
    if(!pcapFile.empty()){   //*从pcap获取数据(代码不执行)
    ...
    else if ("rosbag" == dataType){  //*从rosbag中获取数据(代码不执行)
  ...
  void lidarCallback(boost::shared_ptr<PPointCloud> cld, double timestamp, hesai_lidar::PandarScanPtr scan) // the timestamp from first point cloud of cld
  {
    if(m_sPublishType == "both" || m_sPublishType == "points"){
      // pcl_conversions::toPCL(ros::Time(timestamp), cld->header.stamp);        //原始的
      pcl_conversions::toPCL(ros::Time::now(), cld->header.stamp);   //todo-------------------------------将timestamp改为了ros::Time::now();  （更改了)
      
      sensor_msgs::PointCloud2 output;
      pcl::toROSMsg(*cld, output);
      lidarPublisher.publish(output);    //todo---------------------------------------发布点云-------------------------------
#ifdef PRINT_FLAG
        printf("timestamp: %f, point size: %ld.\n",timestamp, cld->points.size());    //todo------打印每帧点云时间戳和点云大小--------------
#endif        
    }
...
int main(int argc, char **argv)
{
  ...
  // ros::NodeHandle nh("~");
  //todo-----------------------------------------------
  ros::init(argc, argv, "/hesai/pandar");   //话题改为全局
  ros::NodeHandle nh;            //句柄也改为公有
  //todo----------------------------------------------      
  ros::NodeHandle node;
