...
PandarGeneral_Internal::PandarGeneral_Internal(                  //todo---------------------------调用该函数--------------------
...
void PandarGeneral_Internal::Start() {          //todo-----------------------------------------------
  // LOG_FUNC();
  Stop();
...
void PandarGeneral_Internal::Stop() {   //todo---------------------------------------------
  enable_lidar_recv_thr_ = false;
  enable_lidar_process_thr_ = false;
...
void PandarGeneral_Internal::ProcessLiarPacket() {   //todo-----------------------------------------
  // LOG_FUNC();
  ...
  while (enable_lidar_process_thr_) {
    ...
    if (packet.size == PACKET_SIZE || packet.size == PACKET_SIZE + SEQ_NUM_SIZE) {
      ...
      for (int i = 0; i < pkt.header.chBlockNumber; ++i) {
        ...
        CalcXTPointXYZIT(&pkt, i, pkt.header.chLaserNumber, outMsg);    //todo----------------------------------------
        last_azimuth_ = pkt.blocks[i].azimuth;
        last_timestamp_ = pkt.timestamp_point;
      }
    } else {
